...
const AR = (() => {
  ...
  function draw(){
    ...
    const pitchText = pitch === null ? 'unavailable (device gave no tilt data)' : pitch.toFixed(0)+'°';
    ctx.fillText(`hdg ${heading.toFixed(0)}°  ·  pitch ${pitchText}`, 12, canvas.height - 16);
    ...
    // if the selected satellite is above the horizon but outside the current
    // FOV window, show an edge arrow pointing toward it instead of just
    // letting it silently not appear
    if(App.state.selected){
      const sel = App.state.results.find(x => x.name === App.state.selected);
      const onScreen = lastMarkers.some(m => m.r.name === App.state.selected);
      if(sel && !onScreen){
        let dAz = sel.az - heading;
        while(dAz > 180) dAz -= 360;
        while(dAz < -180) dAz += 360;
        const dEl = sel.elev - (pitch ?? 0);
        const angle = Math.atan2(-dEl, dAz); // screen-space direction toward the target
        const edgeR = Math.min(canvas.width, canvas.height)/2 - 40;
        const ax = canvas.width/2 + edgeR*Math.cos(angle);
        const ay = canvas.height/2 + edgeR*Math.sin(angle);
        ctx.save();
        ctx.translate(ax, ay);
        ctx.rotate(angle);
        ...
        ctx.fill();
        ctx.restore();
        ctx.fillStyle = '#FBBF24'; ctx.font = '600 11px Inter, sans-serif'; ctx.textAlign = 'center';
        ctx.fillText(sel.name + ' →', canvas.width/2 + (edgeR-24)*Math.cos(angle), canvas.height/2 + (edgeR-24)*Math.sin(angle));
      }
    }
  }
